...
func TestPrepareHermesHomeSessionStorePersistsAcrossTasks(t *testing.T) {
	...
	sessions, err := prepareHermesHome(firstTask, sharedHome, false, skills, nil, "", store, nil, testLogger())
	if err != nil {
	...
	if _, err := prepareHermesHome(secondTask, sharedHome, false, skills, nil, "", store, nil, testLogger()); err != nil {
...
func TestPrepareHermesHomeSessionStoreIsolatesConversations(t *testing.T) {
	...
	if _, err := prepareHermesHome(homeA, sharedHome, false, skills, nil, "",
		filepath.Join(storeRoot, "agent-1", "default", "issue-a"), nil, testLogger()); err != nil {
	...
	if _, err := prepareHermesHome(homeB, sharedHome, false, skills, nil, "",
		filepath.Join(storeRoot, "agent-1", "default", "issue-b"), nil, testLogger()); err != nil {
...
func TestPrepareHermesHomeWithoutSessionStoreKeepsStateTaskLocal(t *testing.T) {
	...
	sessions, err := prepareHermesHome(hermesHome, sharedHome, false, skills, nil, "", "", nil, testLogger())
	if err != nil {
...
func TestPrepareHermesHomeMigratesTaskLocalSessionDB(t *testing.T) {
	...
	if _, err := prepareHermesHome(hermesHome, sharedHome, false, skills, nil, "", "", nil, testLogger()); err != nil {
		t.Fatalf("prepare pre-store task: %v", err)
	}
	mustWrite(t, filepath.Join(hermesHome, "state.db"), "transcript")
	mustWrite(t, filepath.Join(hermesHome, "state.db-wal"), "uncheckpointed tail")

	if _, err := prepareHermesHome(hermesHome, sharedHome, false, skills, nil, "", store, nil, testLogger()); err != nil {
...
func TestPrepareHermesHomeLinkFailureKeepsTaskLocalDB(t *testing.T) {
	...
	sessions, err := prepareHermesHome(hermesHome, sharedHome, false, skills, nil, "", store, nil, testLogger())
	if err != nil {
