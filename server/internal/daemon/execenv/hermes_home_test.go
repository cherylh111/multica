...
func TestHermesExternalDirsExpandsSanitizedEnv(t *testing.T) {
	...
	skills := []SkillContextForEnv{{Name: "Review Helper", Content: "x"}}
	if _, err := prepareHermesHome(hermesHome, sharedHome, false, skills, env, "", "", nil, testLogger()); err != nil {
		t.Fatalf("prepareHermesHome failed: %v", err)
	}
...
func TestHermesExternalDirsExpandsAgainstSelectedProfileHome(t *testing.T) {
	...
	skills := []SkillContextForEnv{{Name: "Review Helper", Content: "x"}}
	if _, err := prepareHermesHome(hermesHome, profileHome, true, skills, env, "", "", nil, testLogger()); err != nil {
		t.Fatalf("prepareHermesHome failed: %v", err)
	}
...
func TestHermesOverlayEnvPinsHomeAfterDotenvOverride(t *testing.T) {
	...
	hermesHome := filepath.Join(t.TempDir(), "hermes-home")
	skills := []SkillContextForEnv{{Name: "Review Helper", Content: "x"}}
	if _, err := prepareHermesHome(hermesHome, sourceHome, false, skills, nil, "", "", nil, testLogger()); err != nil {
		t.Fatalf("prepareHermesHome failed: %v", err)
	}
...
func TestPrepareHermesHomeFailsOnMissingNamedProfile(t *testing.T) {
	...
	hermesHome := filepath.Join(t.TempDir(), "hermes-home")
	skills := []SkillContextForEnv{{Name: "Review Helper", Content: "x"}}
	if _, err := prepareHermesHome(hermesHome, missingProfileHome, true, skills, nil, "", "", nil, testLogger()); err == nil {
